		  ...
		  int twopass)
{
	...
	/* As per new sequence flow shall be write CSn_CONFIG registers needs
	 * to be set after all the other DDR controller registers are set, then
	 * poll for PHY_INIT_CMPLT = 1 , then wait at least 100us
	 * (micro seconds), then set the MEM_EN = 1
	 */
	for (i = 0; i < DDRC_NUM_CS; i++) {
		if (mod_bnds && !i) {
			ddr_out32(&ddr->csn_cfg[i],
