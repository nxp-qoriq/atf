...
int read_spd(unsigned char chip, void *buf, int len, uintptr_t nxp_i2c_addr)
{
	...
	i2c_write(SPD_SPA0_ADDRESS, 0, 1, &dummy, 1, nxp_i2c_addr);
	ret = i2c_read(chip, 0, 1, buf, 256, nxp_i2c_addr);
	if (!ret) {
		i2c_write(SPD_SPA1_ADDRESS, 0, 1, &dummy, 1, nxp_i2c_addr);
