...
int xspi_ahb_read(uint32_t pcRxAddr, uint32_t *pcRxBuf, uint32_t xSize_Bytes)
{
	...
	pcRxAddr = ((uint32_t)(pcRxAddr + NXP_FLEXSPI_FLASH_ADDR));

	if (pcRxAddr % 4 || (uintptr_t)pcRxBuf % 4) {
		WARN("%s: unaligned Start Address src=%ld dst=0x%p\n",
		     __func__, (pcRxAddr - NXP_FLEXSPI_FLASH_ADDR), pcRxBuf);
	}
...
int xspi_write(uint32_t pcWrAddr, void *pvWrBuf, uint32_t uiLen)
{
	...
	for (i = 0; i < j; i++) {
		INFO("In for loop Buf pvWrBuf=%p, final Buf=%p \
		     xAddr=0x%x offset_buf %d \n", pvWrBuf, Buf, xAddr, xPage1_Len/4);
		xspi_wren(xAddr);
...
int fspi_init(uint32_t base_reg_addr)
{
	uint32_t	mcrx;
	uint32_t	flash_size;
