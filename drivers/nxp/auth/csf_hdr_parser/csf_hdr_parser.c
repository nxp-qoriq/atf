...
static uint32_t get_key(struct csf_hdr *hdr, uint8_t **key, uint32_t *len,
			enum sig_alg *key_type, uintptr_t nxp_dcfg_addr,
			uintptr_t nxp_sfp_addr)
{
	int i = 0;
	uint32_t ret = 0;
	...
	*key_type = RSA;
	...
	key_revoc_flag = get_key_revoc(nxp_sfp_addr);
	 ...
	 */
	sb = check_boot_mode_secure(&mode, nxp_dcfg_addr, nxp_sfp_addr);
	if (sb) {
...
int validate_esbc_header(void *img_hdr, void **img_key, uint32_t *key_len,
			 ...
			 enum sig_alg *algo, uintptr_t nxp_dcfg_addr,
			 uintptr_t nxp_sfp_addr)
{
	...
	ret = get_key(hdr, &key, &klen, algo, nxp_dcfg_addr, nxp_sfp_addr);
	if (ret != 0)
		return -1;
