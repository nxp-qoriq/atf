 ...
 * @retval -1 on error
 */
static void kick_trng(int ent_delay, uintptr_t nxp_caam_addr)
{
	uint32_t val;
...
int hw_rng_instantiate(uintptr_t nxp_caam_addr)
{
	...
	ret = is_hw_rng_instantiated(&state_handle, nxp_caam_addr);
	if (ret) {
		NOTICE("RNG already instantiated\n");
		return 0;
	}
	do {
		kick_trng(ent_delay, nxp_caam_addr);
		ent_delay += 400;
...
int get_rand_bytes_hw(uint8_t *bytes, int byte_len, uintptr_t nxp_caam_addr)
{
	 ...
	 */
	ret_code = is_hw_rng_instantiated(&state_handle, nxp_caam_addr);
	if (!ret_code) {
		INFO("Instantiating the HW RNG\n");

		/* Instantiate the hw RNG */
		ret_code = hw_rng_instantiate(nxp_caam_addr);
		if (ret_code) {
	 ...
	 */
	if (!is_hw_rng_instantiated(&state_handle, nxp_caam_addr)) {
