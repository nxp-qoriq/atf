...
static void config_tz(int num, uintptr_t nxp_caam_addr)
{
	uint32_t jricid;
 ...
 */
static inline void start_jr(int num, uintptr_t nxp_caam_addr)
{
 ...
 */
static int configure_jr(int num, uintptr_t nxp_caam_addr)
{
	int ret;
	void *reg_base_addr;
...
int sec_init(uintptr_t nxp_caam_addr)
{
	...
	ret = configure_jr(DEFAULT_JR, nxp_caam_addr);
	if (ret != 0) {
		ERROR("\nFSL_JR: configuration failure\n");
		return -1;
	}
	/* Do TZ configuration of default JR for sec firmware */
	config_tz(DEFAULT_JR, nxp_caam_addr);

#ifdef CONFIG_RNG_INIT
	/* Instantiate the RNG */
	ret = hw_rng_instantiate(nxp_caam_addr);
	if (ret != 0) {
