...
static int error_handler(int error_code, uintptr_t nxp_dcfg_addr)
{
		      ...
		      uintptr_t nxp_sfp_addr)
{
	...
	if ((fuse_hdr->flags >> (FLAG_DRV0_SHIFT)) & 0x3) {
		...
		if (sfp_read32((void *)(nxp_sfp_addr + SFP_SVHESR_OFFSET))
				& SFP_SVHESR_DRV_MASK)
		      ...
		      uintptr_t nxp_caam_addr)
{
	...
	case PROG_OTPMK_RANDOM:
		...
		if (CHECK_SEC_DISABLED(nxp_dcfg_addr) != 0)
		...
		get_rand_bytes_hw((uint8_t *)otpmk_random,
				   sizeof(otpmk_random), nxp_caam_addr);
...
int provision_fuses(unsigned long long fuse_scr_addr,
		    uintptr_t nxp_gpio1_addr,
		    uintptr_t nxp_gpio2_addr,
		    uintptr_t nxp_gpio3_addr,
		    uintptr_t nxp_gpio4_addr,
		    uintptr_t nxp_dcfg_addr,
		    uintptr_t nxp_sfp_addr,
		    uintptr_t nxp_caam_addr)
{
	...
	if (memcmp(fuse_hdr->barker, barker, sizeof(barker)))
		return error_handler(ERROR_FUSE_BARKER, nxp_dcfg_addr);
	...
	if ((fuse_hdr->flags >> FLAG_DCV0_SHIFT) & FLAG_DEBUG_MASK) {
		...
		ret = prog_debug(fuse_hdr, sfp_ccsr_regs, nxp_sfp_addr);
		if (ret != 0)
			return error_handler(ret, nxp_dcfg_addr);
	}
	...
	if (((fuse_hdr->flags >> FLAG_OTPMK_SHIFT) & PROG_NO_OTPMK) !=
		...
		ret = prog_otpmk(fuse_hdr, sfp_ccsr_regs, nxp_sfp_addr,
				 nxp_dcfg_addr, nxp_caam_addr);
		if (ret != 0)
			return error_handler(ret, nxp_dcfg_addr);
	}
	...
	sfp_cmd_status = sfp_read32(nxp_sfp_addr + SFP_INGR_OFFSET)
			 & SFP_INGR_ERROR_MASK;
	if (sfp_cmd_status != 0)
		return error_handler(ERROR_PROGFB_CMD, nxp_dcfg_addr);
